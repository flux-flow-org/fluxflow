...
export default function Process() {
  return (
    <section id="processo" className="relative overflow-hidden bg-ink py-28 sm:py-32">
      {/* Glow lateral sutil */}
      <div
        aria-hidden="true"
        ...
        >
          ...
          {PROCESSO.map(({ icon: Icon, step, title, desc }) => (
            <motion.li
              key={step}
              variants={fadeUp}
              className="group relative rounded-brand border border-line bg-ink-surface/60 p-7 backdrop-blur transition-all duration-300 hover:-translate-y-1.5 hover:border-flux-purple/50 hover:shadow-glow"
            >
