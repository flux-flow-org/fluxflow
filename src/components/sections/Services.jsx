...
export default function Services() {
  return (
    <section id="servicos" className="relative bg-ink-bg py-20 sm:py-28 lg:py-32">
      ...
      <div className="relative mx-auto max-w-6xl px-5 sm:px-6">
        ...
        <motion.div
          ...
          className="mt-12 grid grid-cols-1 gap-5 sm:mt-16 md:grid-cols-2 lg:grid-cols-4"
        >
          {SERVICOS.map(({ icon: Icon, title, desc }) => (
            <motion.article
              ...
              className="group relative overflow-hidden rounded-brand border border-line bg-ink-surface/70 p-6 backdrop-blur transition-colors duration-300 hover:border-flux-purple/60 sm:p-7"
            >
              {/* Brilho neon que segue o hover */}
              <span className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-500 group-hover:opacity-100 bg-[radial-gradient(circle_at_top_right,rgba(176,142,239,0.14),transparent_60%),radial-gradient(circle_at_bottom_left,rgba(0,229,255,0.08),transparent_55%)]" />
